...
import { Network, Upload, Brain } from "lucide-react";
...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { path: "/models/lr", label: "LR", icon: Brain },
    { path: "/models/rf", label: "RF", icon: Brain },
    { path: "/models/knn", label: "KNN", icon: Brain },
    { path: "/models/svm", label: "SVM", icon: Brain },
    { path: "/models/gbc", label: "GBC", icon: Brain },
  ];
            ...
            <div className="flex items-center gap-1 sm:gap-2 bg-muted/50 rounded-lg p-1 border border-border/40 shadow-sm">
              {navItems.map((item) => {
                ...
                const isActive =
                  ...
                  (item.path.startsWith("/models/") && location.pathname.startsWith(item.path));
                return (
