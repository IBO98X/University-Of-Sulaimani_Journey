...
import { Upload, Brain, Table, Activity } from "lucide-react";
...
export default function ModelPage({
  ...
  // Protocol distribution and quick stats
  const protocolStats = useMemo(() => {
    const counts = new Map<string, { count: number; bytes: number }>();
    for (const p of trafficData) {
      const cur = counts.get(p.protocol) || { count: 0, bytes: 0 };
      counts.set(p.protocol, { count: cur.count + 1, bytes: cur.bytes + p.packet_size });
    }
    const total = trafficData.length;
    const list = Array.from(counts.entries())
      .map(([protocol, v]) => ({ protocol, count: v.count, bytes: v.bytes, percentage: total ? (v.count / total) * 100 : 0 }))
      .sort((a, b) => b.count - a.count);
    return { total, uniqueProtocols: counts.size, items: list };
  }, [trafficData]);

  // Predictions placeholder: using existing protocol mapping as baseline
      ...
      try {
        ...
        try {
          localStorage.setItem(
            `networkAnalyzer_eval_${modelId}`,
            JSON.stringify({ truth: parsed, truthFileName: file.name })
          );
        } catch {}
      } catch (e: any) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem(`networkAnalyzer_eval_${modelId}`);
      if (saved) {
        const parsed = JSON.parse(saved);
        ...
        <div className="container mx-auto px-4 py-6 sm:py-8 lg:py-12">
              ...
              <Card className="border-2 shadow-2xl shadow-primary/10 dark:shadow-primary/20 bg-card/95 backdrop-blur-sm animate-slide-up">
                <CardHeader className="text-center bg-gradient-to-r from-primary/10 via-primary/5 to-transparent">
                  ...
                  <CardDescription className="text-base sm:text-lg mt-2">
                    Upload and analyze a PCAP file to view {title}
                  </CardDescription>
                </CardHeader>
          ...
          <div className="mb-6 sm:mb-8 animate-fade-in">
            ...
            {filename && (
              <p className="text-sm text-muted-foreground mt-2 ml-12">
                Analyzed file: <span className="font-semibold">{filename}</span>
              </p>
            )}
            <p className="text-sm text-muted-foreground mt-2 ml-12 max-w-3xl">{description}</p>
          </div>
          ...
          <Card className="border-2 shadow-2xl shadow-primary/10 dark:shadow-primary/20 bg-card/95 backdrop-blur-sm mb-6 animate-slide-up">
            <CardHeader className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent">
              <CardTitle className="text-xl sm:text-2xl font-bold">Traffic Overview</CardTitle>
              <CardDescription>Packet counts and protocol distribution</CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                <div className="p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Total Packets</p>
                  <p className="text-lg font-bold">{protocolStats.total.toLocaleString()}</p>
                </div>
                <div className="p-3 rounded-lg border">
                  <p className="text-xs text-muted-foreground">Unique Protocols</p>
                  <p className="text-lg font-bold">{protocolStats.uniqueProtocols}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {protocolStats.items.map((it) => (
                  <div key={it.protocol} className="p-3 rounded-lg border">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium">{it.protocol}</span>
                      <Badge variant="outline">{it.count.toLocaleString()} pkts</Badge>
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {(it.percentage).toFixed(1)}% • {it.bytes.toLocaleString()} bytes
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card className="border-2 shadow-2xl shadow-primary/10 dark:shadow-primary/20 bg-card/95 backdrop-blur-sm mb-6 animate-slide-up">
            <CardHeader className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent">
              ...
              <CardDescription>
                Upload ground truth as CSV (id,true_class or id,true_protocol) or JSON array
              </CardDescription>
            </CardHeader>
            <CardContent className="pt-6">
              ...
              {parseError && <p className="text-sm text-red-500 mt-3">{parseError}</p>}
            </CardContent>
          </Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {evaluation.perClass.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <Badge variant="outline" style={{ borderColor: info.color }}>
                            Class {m.class}
                          </Badge>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div>
                            ...
                            <span className="font-semibold">{(m.precision * 100).toFixed(1)}%</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">R:</span>{" "}
                            <span className="font-semibold">{(m.recall * 100).toFixed(1)}%</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">F1:</span>{" "}
                            <span className="font-semibold">{(m.f1 * 100).toFixed(1)}%</span>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <tbody>
                    {CLASSES.map((r) => (
                      <tr key={r} className="border-t">
                        ...
                        {CLASSES.map((c) => (
                          <td key={r + "-" + c} className="p-2 text-center">
                            {evaluation.matrix[r][c]}
                          </td>
                        ))}
                      </tr>
                    ))}
          ...
          {/* Packets table */}
          <Card className="border-2 shadow-xl bg-card/95 mt-6">
            <CardHeader>
              <div className="flex items-center gap-2">
                <Table className="h-5 w-5" />
                <CardTitle>Packets</CardTitle>
              </div>
              <CardDescription>All parsed packets used for analysis</CardDescription>
            </CardHeader>
            <CardContent className="pt-4 overflow-auto">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr>
                    ...
                    <th className="p-2 text-left">Dst IP</th>
                ...
                <tbody>
                  {trafficData.map((p) => (
                    <tr key={p.id} className="border-t">
                      ...
                      <td className="p-2">{p.source_ip}:{p.source_port}</td>
                      <td className="p-2">{p.dest_ip}:{p.dest_port}</td>
                      <td className="p-2 text-right">{p.packet_size}</td>
