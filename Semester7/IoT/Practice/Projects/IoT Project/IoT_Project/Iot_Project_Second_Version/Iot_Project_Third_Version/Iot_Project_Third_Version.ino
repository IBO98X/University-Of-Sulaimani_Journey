...
const int yellowDryPin = 3; // <-- CHANGED: Set to pin 2 as you asked
const int redWetPin = 4;    // <-- This is already pin 4
...
void setup() {
  ...
  delay(500); // Give it time to get to position
  servo1.detach(); // <-- ADDED: Relax the servo at start to stop buzz
...
void runSorterLogic() {
  ...
  if (distance < defaultDistanceCM && distance > 1) {
    ...
    // --- SERVO FIX: WAKE UP THE SERVO ---
    servo1.attach(8); // <--- ADDED: Wake up servo
    delay(10); // Small delay for servo to initialize
    ...
    if (wetnessPercent > wetnessThreshold) {
      ...
      digitalWrite(redWetPin, HIGH); // <-- ADDED THIS FIX
      ...
      digitalWrite(yellowDryPin, HIGH); // <-- ADDED THIS FIX
    ...
    // --- SERVO FIX: After sorting, return to neutral and relax
    servo1.write(130);
    delay(500); // <--- ADDED: Give servo time to move back
    servo1.detach(); // <--- ADDED: Relax the servo
    Serial.println("---");
